...
public class RestTemplateDemoController {
    ...
    @Value("${host.pmsystem.admin}")
    private String HOST_PMSYSTEM_ADMIN;

    @ApiOperation("getForEntity url")
    @RequestMapping(value = "/get/{id}",method = RequestMethod.GET)
    @ResponseBody
    public Object getForEntity(@PathVariable long id){
        String url=HOST_PMSYSTEM_ADMIN;
    ...
    public Object getForEntity2(@PathVariable long id){
        String url=HOST_PMSYSTEM_ADMIN+"/brand/{id}";
    ...
    @ApiOperation("getForEntity jsonBody")
    @RequestMapping(value = "/post",method = RequestMethod.POST)
    @ResponseBody
    public Object postForEntity(@RequestBody PmsBrand brand) {
        String url = HOST_PMSYSTEM_ADMIN + "/brand/create";
